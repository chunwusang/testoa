...
class Flowagent_custfina  extends Rockflow
{
	...
	protected function flowinit()
	{
		...
		$this->statuszf		= array('否','<font color=green>是</font>');
	}
	...
	public function flowsavebefore($arr)
	{
		...
		if($orderid>0){
			...
			if($moneys+$money > $orderrs->money)return '收款金额不能超过订单金额';
		}
		if($arr->ispay==1){
			if(isempt($arr->paydt))return '收款时间不能为空';
		}else{
			$data['paydt'] = null;
		}

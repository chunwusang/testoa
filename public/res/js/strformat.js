...
var strformat = {
	...
	showqp:function(type,name,dt,cont,nuid, fase,rnd){
		...
		nr+='			<td>';
		nr+='			<div onclick="im.clickqipao(this,event,\''+rnd+'\')" oncontextmenu="im.rightqipao(this,event,\''+rnd+'\')" ontouchstart="touchobj=this" id="qipaocont_'+rnd+'" rand="'+rnd+'" class="qipaocont qipaocont'+type+'">'+str+'</div>';
		nr+='			</td>';
	...
	contshozt:function(d, lj){
		...
		if(js.isimg(d.fileext)){
			s='<img src="'+d.imgpath+'" fid="'+d.fileid+'">';
		}else{
			s=''+d.filename+'<br><a href="javascript:;" onclick="strformat.openfile(\''+d.filenum+'\',\''+d.filesizecn+'\')">下载</a>&nbsp;&nbsp;<a href="javascript:;" onclick="strformat.openfiles(\''+d.filenum+'\',0)">预览</a>&nbsp;'+d.filesizecn+'';
	...
	openfile:function(num,dx){
		js.confirm('确定要下载此文件吗？大小['+dx+']！<br>你也可以<a href="javascript:;" onclick="strformat.openfiles(\''+num+'\',0)">[预览]</a>此文件。', function(jg){
			if(jg=='yes')strformat.openfiles(num, 1);
		});
	},
	onopenfile:function(){
		return false;
	},
	openfiles:function(num,glx){
		js.alert('none');
		js.loading('处理中...');
		js.ajax('/api/we/file_down/'+cnum+'', {'num':num,'glx':glx},function(ret){
			var da = ret.data;
			if(strformat.onopenfile(da,glx))return;
			if(glx==0 && da.isimg=='1'){
				strformat.imgview(da.upurl);
			}else{

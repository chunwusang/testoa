<?php
/**
*	应用-会议
*	主页：http://www.rockoa.com/
*	软件：信呼OA云平台
*	作者：雨中磐石(rainrock)
*	时间：2017-12-05
*/

use Illuminate\Support\Facades\Schema;
use Illuminate\Database\Schema\Blueprint;
use Illuminate\Database\Migrations\Migration;

class CreateAgentMeetTable extends Migration
{
    /**
     * Run the migrations.
     * @return void
     */
	
	private $tablename = 'meet'; 
	
	 
    public function up()
    {
        Schema::create($this->tablename, function (Blueprint $table) {
			$table->comment = '会议';
			
            $table->increments('id');
			$table->integer('cid')->default(0)->comment('单位company.id');
			$table->integer('uid')->default(0)->comment('平台用户users.id');
			$table->integer('aid')->default(0)->comment('平台单位用户usera.id');
			$table->tinyInteger('status')->default(1)->comment('状态');
			$table->tinyInteger('isturn')->default(0)->comment('是否提交');
			
			$table->integer('hysid')->default(0)->comment('会议室id');
			$table->string('hyname',100)->default('')->comment('会议室名称');
            $table->string('title',100)->default('')->comment('会议标题');
			$table->string('content',1000)->default('')->comment('内容');
			
			$table->datetime('startdt')->nullable()->comment('开始时间');
			$table->datetime('enddt')->nullable()->comment('截止时间');
			
			$table->tinyInteger('state')->default(0)->comment('会议状态@0|正常,1|会议中,2|结束,3|取消');
			
			$table->string('joinid',1000)->default('')->comment('参会人');
			$table->string('joinname',1000)->default('')->comment('参会人ID');
			
			$table->string('jyname',100)->default('')->comment('会议纪要人');
			$table->string('jyid',100)->default('')->comment('会议纪要人ID');
			$table->datetime('optdt')->nullable()->comment('操作时间');
			
			$table->index('cid');
			$table->index('aid');
        });
    }

    /**
     * Reverse the migrations.
     *
     * @return void
     */
    public function down()
    {
        Schema::dropIfExists($this->tablename);
    }
}
